...
import com.yunruiinfo.iclass.student.bean.Update;
...
public class UpdateManager {
		...
		new Thread(){
			public void run() {
				...
				try {
					...
					params.addQueryStringParameter("act", "app_new");
					params.addBodyParameter("type", "0");
					String json = http.sendSync(HttpMethod.GET, URLs.YR_API, params).readString();
					Result result = JsonUtils.fromJson(json, Result.class);
					Update update = result.version;
					msg.what = 1;
					msg.obj = update;
	...
	private Runnable mdownApkRunnable = new Runnable() {	
		...
		public void run() {
			try {
				String apkName = "iStudent_"+mUpdate.getVersionName()+".apk";
				String tmpApk = "iStudent_"+mUpdate.getVersionName()+".tmp";
				//判断是否挂载了SD卡
				String storageState = Environment.getExternalStorageState();		
				if(storageState.equals(Environment.MEDIA_MOUNTED)){
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/iStudent/Update/";
